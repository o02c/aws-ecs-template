...
4. **Docker build & push**：`apps/{nginx, user-api, admin-api}` を `linux/amd64` でビルドし ECR へ push。aws-for-fluent-bit:init-latest は ECR pull-through cache が初回 pull 時に自動キャッシュするので手動ミラーなし。
